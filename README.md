...
```
sort transaction_data_daily_event_log_20190129.dat | uniq | wc
```
...
```
ggrep -P "failure" -n transaction_data_daily_event_log_20190129.dat
```

Upon review, we would like to only view failures with error code SYSOFFLINE or WEAKSIGNAL.

```
ggrep -P 'SYSOFFLINE|WEAKSIGNAL' -n transaction_data_daily_event_log_20190129.dat
```
...
```
ggrep -P '(?<!@)"{0,6}[a-zA-Z_][^,]'-c users.csv
